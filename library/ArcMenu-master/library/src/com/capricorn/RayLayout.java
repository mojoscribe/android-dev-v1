...
public class RayLayout extends ViewGroup {
	...
	private  Rect computeChildFrame(final boolean expanded, final int paddingRight, final int childIndex,
            final int gap, final int size) {
        final int left = this.getWidth() -  (expanded ? (paddingRight + childIndex * (gap + size) + gap) : ((paddingRight - size) / 2));
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < childCount; i++) {
			Rect frame = computeChildFrame(mExpanded, 0, i, mChildGap, mChildSize);
